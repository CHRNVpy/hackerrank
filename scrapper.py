...
def get_url_list(sheet_name: str, start_row: int):
    ...
    creds = ServiceAccountCredentials.from_json_keyfile_name('YOUR_JSON_API_KEY', scope)
    ...
    # print each value in the first column
    # url_list = []
    ...
    # iterate over values in the first column starting from the third row
    url_list = []
    for row_num in range(start_row, num_rows + 1):
        value = sheet.cell(row_num, 1).value
        url_list.append(value)
    ...
    def get_code(question_number):
        ...
        if soup_code.find('span', {'class': "c-fawirw"}).find('span').text == 'Partial' or 'Correct':
            ...
            copy_code_button = driver.find_element(By.CSS_SELECTOR,
                                                   '.report-playback__copy-code > span:nth-child(1) > div:nth-child(1) > svg:nth-child(1)').click()
...
# url = 'https://www.hackerrank.com/work/tests/1510939/candidates/completed/49421918/report/'
# print(get_data(url))

dd = ['https://www.hackerrank.com/x/tests/1510939/candidates/49272112/report', 'https://www.hackerrank.com/x/tests/1510939/candidates/49219581/report', 'https://www.hackerrank.com/x/tests/1510939/candidates/49216054/report']
...
""" Here we can start to scrapp """

urls = get_url_list(sheet_name='YOUR_SHEET_NAME', start_row=1) #you can change start row number if needed

for url in urls: #if errors occurs after scan, change urls to my_list, this will scan error_urls.txt(unscanned urls)
    try:
        res = get_data(url, login='YOUR_LOGIN', password='YOUR_PASSWORD')
